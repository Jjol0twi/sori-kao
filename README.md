...
> 한국어 단어/문장의 **소리 느낌(음운 특징)** 을 분석해 어울리는 **기호 카모지**를 추천하고, **그 이유까지 설명**하는 Python 앱.
> Rule-based, explainable kaomoji recommender driven by Korean phonetic symbolism — no AI/LLM, fully offline.

## 무엇을 하는가

`아싸`, `으으`, `ㅋㅋㅋ`, `하...`, `빠샤`처럼 감정 단어가 없어도, 입력의 소리와 형태(밝은/어두운 모음, 된소리·거센소리, 받침, 반복, 문장부호)를 수치화해 감정·상황 카테고리를 점수화하고, 카테고리에 연결된 카모지를 추천한다.
...
- **음운 특징 중심**: 단순 키워드 매칭이 아니라 14차원 음운 특징 벡터로 10개 카테고리(응원·기쁨·당황·분노·긴장·피곤·장난·사과·감사·집중)의 Top-3를 계산한다.
...
입력 → 전처리 → 한글 분해 → 음운 특징 추출(14차원)
     → NumPy 점수 계산 → Top-3 카테고리 → 카모지 추천 → 추천 이유 출력
```
...
uv가 받는 Python에는 Tk 8.6이 포함된다. `.python-version`(3.13) 덕분에 버전이 자동 선택된다.
...
Tk 버전 확인: `python -c "import tkinter; print(tkinter.TkVersion)"` → **8.6 이상**이어야 한다.
...
- `하암`처럼 하품을 뜻하는 관습적 의성어는 밝은 ㅏ 모음 때문에 `하하`와 음운적으로 가깝다. 이런 예외는 `auxiliary_patterns`로 보정하되, 낮은 신뢰도로 표시한다.
