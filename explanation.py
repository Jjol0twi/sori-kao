...
_WEAK_SIGNAL_NOTE = (
    "뚜렷한 음운 인상이 약합니다(의성어·의태어가 아닌 일반 표현일 수 있음). "
    "아래 해석은 약한 경향이며 참고용입니다."
)
...
def build_explanation(score_result) -> Explanation:
    ...
    # 정보량(유표) 신호가 약한 입력은 '뚜렷한 인상 없음'을 우선 알려 과잉 해석을 막는다.
    if getattr(score_result, "weak_signal", False):
        note = _WEAK_SIGNAL_NOTE
    elif score_result.confidence == "low":
